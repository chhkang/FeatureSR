...
parser.add_argument("--loss_type", type=str, default='ssim_loss', help="loss type in training")
...
for feature in feature_types:
    ...
    if opt.model_name == 'VDSR':
        ...
        model_path = './result/{}/CARN_p2x{}/model_latest.pth'.format(opt.loss_type,opt.rescale_factor)
        checkpoint = torch.load(model_path)
        netG.load_state_dict(checkpoint,strict = True)
    ...
    for batch_idx, (lr_f, _) in enumerate(dataloader):
        lr_f = lr_f.cuda()
        ...
        if(batch_idx%4==0):
            for i in range(0,4):
                low = sr_f[16*i,:,:,:]
                for j in range(1,16):
                    low = np.concatenate((low,sr_f[16*i+j,:,:,:]),0)
                if(i == 0):
                    pic = low
                else:
            ...
            for i in range(0,4):
                ...
                pic = np.concatenate((pic,low),1)
            if(batch_idx%4==3):
                cv2.imwrite(os.path.join(out_pth,"SR_{}.png".format(int(batch_idx/4))),pic)
