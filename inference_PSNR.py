...
for feature in feature_types:
    ...
    for batch_idx, (lr_f, hr_f) in enumerate(dataloader):
        ...
        sr_f = sr_f.astype(np.uint8).astype(float)

        hr_f = hr_f.cuda()
        hr_f = torch.clamp(hr_f,0,1)
        hr_f = torch.reshape(hr_f,shape)*255
        hr_f = hr_f.cpu().detach().numpy()
        hr_f = hr_f.astype(np.uint8).astype(float)

        bic = torch.clamp(bicu(lr_f),0,1)
        bic = torch.reshape(bic,shape)*255
        bic = bic.cpu().detach().numpy() ###256x1x50x68
        bic = bic.astype(np.uint8).astype(float)

        bic_psnr += utils.psnr(bic,hr_f)
        sr_psnr += utils.psnr(sr_f,hr_f)
    print(bic_psnr/len(dataloader),sr_psnr/len(dataloader))
