...
def myRound(x): # 양수와 음수에 대해 0을 대칭으로 rounding
  abs_x = abs(x)
  val = np.uint16(abs_x + 0.5)
  val2 = np.choose(x < 0,[val, val*(-1)])
  return val2

def myClip(x, maxV):
  val = np.choose(x > maxV,[x, maxV])
  return val
...
for iter in range(0, 100):
  ...
  image = cv2.imread('./dataset/validset_100/000000'+ image_file_number +'.jpg')
  p2_feature_img = Image.open('/home/ubuntu/JH/exp1/dataset/valid/p2/COCO_val2017_000000{}_p2.png'.format(image_file_number))
  p2_feature_arr = np.array(p2_feature_img)
  p2_feature_arr_round = myRound(p2_feature_arr)

  p3_feature_img = Image.open('/home/ubuntu/JH/exp1/dataset/valid/p3/COCO_val2017_000000{}_p3.png'.format(image_file_number))
  p3_feature_arr = np.array(p3_feature_img)
  p3_feature_arr_round = myRound(p3_feature_arr)
  
  p4_feature_img = Image.open('/home/ubuntu/JH/exp1/dataset/valid/p4/COCO_val2017_000000{}_p4.png'.format(image_file_number))
  p4_feature_arr = np.array(p4_feature_img)
  p4_feature_arr_round = myRound(p4_feature_arr)

  p5_feature_img = Image.open('/home/ubuntu/JH/exp1/dataset/valid/p5/COCO_val2017_000000{}_p5.png'.format(image_file_number))
  p5_feature_arr = np.array(p5_feature_img)
  p5_feature_arr_round = myRound(p5_feature_arr)

  p6_feature_img = Image.open('/home/ubuntu/JH/exp1/dataset/valid/p6/COCO_val2017_000000{}_p6.png'.format(image_file_number))
  ...
  # MSB 코드
  shift_bit = 8 - bitDepth
  recon_p2 = ((recon_p2+1)/math.pow(2,shift_bit)) - 1
  recon_p3 = ((recon_p3+1)/math.pow(2,shift_bit)) - 1
  recon_p4 = ((recon_p4+1)/math.pow(2,shift_bit)) - 1
  recon_p5 = ((recon_p5+1)/math.pow(2,shift_bit)) - 1
  recon_p6 = ((recon_p6+1)/math.pow(2,shift_bit)) - 1
  ...
  print(torch.mean(tensor_value),torch.mean(tensor_value5))
  # MSB 코드 끝
