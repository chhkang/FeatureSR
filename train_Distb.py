...
parser.add_argument("--loss_type", type=str, choices= ["MSE", "L1", "SmoothL1","vgg_loss","ssim_loss","adv_loss","lpips","KLD","WSD","JSD","HD","TVD"], default='ssim_loss', help="loss type in training")
...
def main():
    ...
    file_pth = './result/{}/CARN_p2x{}'.format(opt.loss_type,opt.rescale_factor)
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            if (opt.loss_type =='adv_loss' or opt.loss_type == 'lpips'):# and epoch> int(opt.nEpochs/2):
                ...
                optimizerG.step()
                if batch_idx%30 == 0:
                    print("===> Epoch[{}]({}/{}): {} Loss: {:.5}".format(epoch, batch_idx, len(train_loader),opt.loss_type,loss.item()))
        if not os.path.exists('./result/{}/{}_{}x{}'.format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor)):
            os.makedirs('./result/{}/{}_{}x{}'.format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor))
        ...
        for batch_idx, (lr_f, hr_f) in enumerate(test_loader):
            ...
            if(batch_idx<10):
                if(epoch == 1):
                    cv2.imwrite("./result/{}/{}_{}x{}/LR_{}.png".format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor,batch_idx),lr_f)
                    cv2.imwrite("./result/{}/{}_{}x{}/HR_{}.png".format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor,batch_idx),hr_f)
                cv2.imwrite("./result/{}/{}_{}x{}/SR_{}.png".format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor,batch_idx),sr_f)
        ...
        model_out_latest_path = './result/{}/{}_{}x{}/model_latest.pth'.format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor)
        torch.save(netG.state_dict(), model_out_latest_path)
    
def save_checkpoint(model, epoch):
    print('Saving Networks..')
    model_out_path = './result/{}/{}_{}x{}/model.pth'.format(opt.loss_type, opt.model, opt.feature_type,opt.rescale_factor)
